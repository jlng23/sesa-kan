...
from util import losses, meters
...
def train_reg_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                    mixup_fn: Optional[Mixup] = None, log_writer=None,
                    args=None):
...
def train_multi_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # Calculate and log the Mean Squared Error
        # mse_value = mean_squared_error(outputs, targets).item()
        meter.update(outputs.detach().cpu().numpy(), targets.detach().cpu().numpy())
        get_meters = meter.get_metrics()
        metric_logger.update(mse=get_meters['mae'].item())
        mse_value_reduce = misc.all_reduce_mean(get_meters['mae'].item())
...
def evaluate_multi(data_loader, model, device, criterion, name_meter):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        metric_logger.meters['acc'].update(get_meters['acc'].item(), n=batch_size)
        metric_logger.meters['f1'].update(get_meters['f1'].item(), n=batch_size)
        
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print('*  loss {losses.global_avg:.3f} MAE {mse.global_avg:.3f} R2 {r2.global_avg:.3f} ACC {acc.global_avg:.3f} F1 {f1.global_avg:.3f}'
          .format( losses=metric_logger.loss, mse=metric_logger.mae, r2=metric_logger.r2, acc=metric_logger.acc, f1=metric_logger.f1))
...
# from torchmetrics.regression import MeanSquaredLogError, R2Score, RelativeSquaredError

@torch.no_grad()
def evaluate_reg(data_loader, model, device):
    # criterion = torch.nn.MSELoss()
    criterion = losses.get_loss_criterion("mse")
    meter = meters.get_meter('RegressionMeter')
    # r2_score = R2Score().to(device)
    # relative_squared_error = RelativeSquaredError().to(device)
    # mean_sle = MeanSquaredLogError().to(device)
